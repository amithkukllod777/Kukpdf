...
- [x] All tool pages now run real processing — on-device (merge/split/rotate/delete/reorder/duplicate/insert-blank/watermark/page-numbers/compress/repair/sign/OCR/searchable-PDF/password-protect/unlock/annotate) plus the two AI tools (Summarize PDF, Ask PDF) via the shared backend LLM. No fake successes remain.
- [x] AI tools (Summarize PDF + Ask PDF): call the shared kukbook-erp backend (`/api/kukpdf/ai/*`, `invokeLLM`). **Subscription-gated + rate-limited** per the owner's requirement: signed-out = blocked, free (signed-in) = 3 AI calls/day trial, Premium = 50/day, Business = 100/day (server-enforced via a reserve-then-release per-user/day counter in `kukpdf_ai_usage`; a blocked or failed call never burns quota). PDF text is extracted on-device (pdfjs) and sent to the LLM; a text-less scan is refused with a "run OCR first" note. Paywall/upsell opens `/pdf/pricing`. On-device tools stay **unlimited** (no server cost). Profile Pro card shows real plan + remaining AI quota. Backend in kukbook-erp PR #943 (migration `kukpdf_ai_usage_migrate.cjs` to run on the AWS DB); goes live when that PR deploys.
